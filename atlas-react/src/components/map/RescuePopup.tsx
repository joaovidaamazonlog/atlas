      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-2 p-3">
        ...
        {!state.loading && state.results.length === 0 && (
          <p className="text-xs text-atlas-muted text-center py-4">
            {t('rescue.no_results', { radius: RESCUE_RADIUS_KM })}
          </p>
        )}

        {!state.loading && state.results.map(({ partner, distanceKm, bonus }) => (
          <div key={partner.store_id} className="rounded-lg bg-atlas-darker p-3 flex flex-col gap-1.5">
            <span className="text-sm font-semibold text-atlas-light leading-tight">
              {partner.name}
            </span>
            <p className="text-xs text-atlas-muted">
              {t('rescue.distance')}{' '}
              <span className="text-atlas-light font-medium">{distanceKm} {t('rescue.km')}</span>
              {' · '}
              {t('rescue.bonus')}{' '}
