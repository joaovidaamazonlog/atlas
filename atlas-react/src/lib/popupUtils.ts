...
function _popupActive(p: Partner, routeOriginActive = false): string {
  return _popupWrapper(`
    ...
    ${_table(
      _row(i18n.t('popup.store_id'), p.store_id) +
      _row(i18n.t('popup.status'), p.status) +
      _row(i18n.t('popup.bucket'), p.bucket_ade) +
      _row(i18n.t('popup.delivery_station'), p.delivery_station) +
      _row(i18n.t('popup.launch_date'), p.launch_date) +
      _row('HCP Initiatives', p.hub_delivey_initiatives) +
      _row('HCP Host Partner', p.HCP_host_partner) +
      _row('HCP Rate Card', p.HCP_rate_card) +
      _row(i18n.t('popup.radius'), p.radius + ' m') +
      _row(i18n.t('popup.capacity'), p.capacity + ' pkgs'),
    )}
    ${_hr()}
...
function _popupInactive(p: Partner, routeOriginActive = false): string {
  ...
  return _popupWrapper(`
    ...
    ${_table(
      ...
      _row(i18n.t('popup.cap_suggestion'), capSuggestion + ' pkgs') +
      _row(i18n.t('popup.radius_suggestion'), radiusSuggestion + ' m'),
    )}
    ${_actionButtons(p, routeOriginActive)}
  `);
}
...
function _popupOnboarding(p: Partner, routeOriginActive = false): string {
  return _popupWrapper(`
    ...
    ${_hr()}
    ${_table(
      _row(i18n.t('popup.cap_suggestion'), _opt(p).cap_suggestion + ' pkgs') +
      _row(i18n.t('popup.radius_suggestion'), _opt(p).radius_suggestion + ' m'),
    )}
    ${_actionButtons(p, routeOriginActive)}
  `);
}
