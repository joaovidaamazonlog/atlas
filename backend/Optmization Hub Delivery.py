...
class OptimizationHubDelivery:
    ...
    def run_all_analyses(self):
        ...
        results['gaps'] = self.identify_coverage_gaps()
    ...
    def identify_coverage_gaps(self, grid_size=0.01):
        """3. Sugestão de novos parceiros (Gaps de cobertura)"""
        print("Identificando gaps de cobertura...")
        # Criar grid simples baseado em lat/lon
        self.df_packages['grid_lat'] = np.round(self.df_packages['latitude'] / grid_size) * grid_size
        self.df_packages['grid_lon'] = np.round(self.df_packages['longitude'] / grid_size) * grid_size
        
        grid_counts = self.df_packages.groupby(['grid_lat', 'grid_lon']).size().reset_index(name='package_count')
        
        # Verificar se cada célula do grid tem parceiro próximo
        tree = KDTree(self.df_partners[['lat', 'lon']].values)
        distances, _ = tree.query(grid_counts[['grid_lat', 'grid_lon']].values, k=1)
        grid_counts['dist_to_nearest_partner'] = distances * 111320
        
        # Gaps: Alta densidade e longe de parceiros (ex: > 2km)
        gaps = grid_counts[(grid_counts['dist_to_nearest_partner'] > 2000) & (grid_counts['package_count'] > 50)]
        return gaps
...
if __name__ == "__main__":
    ...
    # 3. Gaps
    gaps = hub.identify_coverage_gaps()
    ...
    # 6. Simulação
    sim = hub.simulate_scenario(partners_to_remove=['None'])
