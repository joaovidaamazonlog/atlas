...
class JsonGenerator:
    ...
    def _create_popup_html(row: pd.Series) -> str:
        ...
        if pd.notna(row.get("Phone")):
            ...
            link_whatsapp = f'<br><a href="https://wa.me/{phone_number}" target="_blank"><i class="fa fa-whatsapp"></i></a>'
            

        return f'<div style="width: 350px; max-height: 400px; font-size: 12px;">{info_html}<hr class: "my-2">{link_salesforce}<br>Enviar menssagem {row.get("Phone")} {link_whatsapp}><hr class: "my-2">{metrics_html}</div>'
