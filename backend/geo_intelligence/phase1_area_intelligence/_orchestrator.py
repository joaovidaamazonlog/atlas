...
from geo_intelligence.phase1_area_intelligence.profile_builder import build_reference_profiles
...
from geo_intelligence.phase1_area_intelligence.feature_engineer import run_feature_engineering, NUMERIC_FEATURES
...
from geo_intelligence.geo_config import EXIT_REASON_MAP, MIN_TENURE_DAYS_FOR_PROFILE
...
def run_area_intelligence(
    ...
    territories_index: dict,  # {territory_id: {h3_ids_r8: [...], h3_ids_r9: [...], current_partners: int, ideal_slots: int}}
    turso_url: str,
    turso_auth_token: str,
    ibge_census_path: str = "",
    models_dir: str = "models",
    partner_profiles: list | None = None,
    partner_data_df: pd.DataFrame | None = None,  # NEW: raw partner data for ingest_partners
    reference_profiles=None,  # NEW: pre-built ReferenceProfiles (optional override)
    ...
    # ------------------------------------------------------------------
    # Step 1b: Ingest partners → list[PartnerProfile]
    # ------------------------------------------------------------------
    if partner_data_df is not None and not partner_data_df.empty:
        logger.info("[Phase 1] Station %s — ingesting partners.", station_code)
        partner_profiles = ingest_partners(partner_data_df)
        logger.info("[Phase 1] %d partner profiles ingested.", len(partner_profiles))
    elif partner_profiles is None:
        partner_profiles = []
    ...
    # ------------------------------------------------------------------
    # Step 3b: Build reference profiles (Institutional Memory)
    # ------------------------------------------------------------------
    if reference_profiles is None and partner_profiles:
        logger.info("[Phase 1] Building reference profiles.")
        import numpy as np
        cells_features = {
            c.h3_id: np.array([float(getattr(c, f) or 0.0) for f in NUMERIC_FEATURES])
            for c in cells
        }
        reference_profiles = build_reference_profiles(
            ...
            min_tenure_days=MIN_TENURE_DAYS_FOR_PROFILE,
            station_code=station_code,
        )
        metrics["profile_coverage"] = reference_profiles.profile_coverage
        metrics["low_coverage_warning"] = reference_profiles.low_coverage_warning
        metrics["is_global_fallback"] = reference_profiles.is_global_fallback
    else:
        cells_features: dict = {}
    ...
    classifications, classifier_metrics, umap_model = classify_cells(
        cells=cells,
        station_code=station_code,
        models_dir=models_dir,
        partner_profiles=partner_profiles,  # NEW: fills umap_embedding on profiles
    )
    metrics["classifier"] = classifier_metrics
    metrics["umap_params"] = classifier_metrics.get("umap_params")
    metrics["n_clusters"] = classifier_metrics.get("n_clusters")
    metrics["low_quality_clustering"] = classifier_metrics.get("low_quality_clustering")

    # Populate features on partner profiles from normalized cell features
    if partner_profiles and cells_features:
        cell_feature_map = {c.h3_id: c for c in cells}
        for pp in partner_profiles:
            if pp.h3_id_r8 in cell_feature_map:
                cell = cell_feature_map[pp.h3_id_r8]
                pp.features = {f: float(getattr(cell, f) or 0.0) for f in NUMERIC_FEATURES}
    ...
    # territories_index: {territory_id: {h3_ids_r8: [...], h3_ids_r9: [...], current_partners: int, ideal_slots: int}}
    # Also supports legacy key "h3_ids" for backward compatibility.
    ...
    for territory_id, info in territories_index.items():
        # Support both new (h3_ids_r8) and legacy (h3_ids) keys
        territory_h3_ids_r8[territory_id] = info.get("h3_ids_r8") or info.get("h3_ids", [])
        territory_h3_ids_r9[territory_id] = info.get("h3_ids_r9", [])
        current_partners[territory_id] = info.get("current_partners", 0)
        ideal_slots[territory_id] = info.get("ideal_slots", 0)

    # Keep a unified h3_ids view (r8) for internal use
    territory_h3_ids = territory_h3_ids_r8

    # If territories_index is empty, create a single territory from all H3 cells
    if not territory_h3_ids_r8:
        logger.warning(
            "[Phase 1] territories_index is empty — grouping all H3 cells into one territory."
        )
        territory_id = f"{station_code}_default"
        territory_h3_ids_r8[territory_id] = h3_ids
        territory_h3_ids_r9[territory_id] = []
        territory_h3_ids = territory_h3_ids_r8
    ...
    selected_territories = select_areas(
        ...
        territory_h3_ids_r8=territory_h3_ids_r8,
        territory_h3_ids_r9=territory_h3_ids_r9,
