...
class HexagonGenerator:
    ...
    def load_packages_data(self):
        ...
        if 'hex' not in df.columns:
            # Usar resolução 7 conforme solicitado
    ...
    def export_csv(self, output_filename="hexagons_res7.csv"):
        """Exporta CSV com station_code, total_packages, list_of_CEPs, latitude, longitude por hex."""
        dest = Path(Config.DEST_FOLDER)
        dest.mkdir(exist_ok=True)
        output_path = dest / output_filename
        ...
        with open(output_path, "w", encoding="utf-8", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["station_code", "total_packages", "latitude", "longitude"])
            for _, row in self.demand_df.iterrows():
                hex_id = row['hex']
                station_code = row['station_code']
                total_packages = int(row['avg_demand'])
                lat, lon = h3.cell_to_latlng(hex_id)
                writer.writerow([station_code, total_packages, lat, lon])
