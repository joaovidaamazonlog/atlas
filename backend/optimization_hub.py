...
class OptimizationService:
    ...
    def _load_data(self):
        ...
        raw_grouped["avg_demand"] = raw_grouped["qty"].astype(int)
    ...
    def run(self):
        ...
        for base in self.demand_df.station_code.unique():
            ...
            cluster_metrics, hex_to_bucket = regionalization.regionalize_demand_balanced(station_code=base, partners=all_partners, n_clusters=n_buckets, demand_map=orig_dem)
            ...
            print(f"   ✅ Base {base} concluída. (Total esperado: {len(all_partners)}, Ativos: {len(p_active)}, Novos: {len(p_new)})")
